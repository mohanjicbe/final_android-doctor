...
import com.orane.docassist.fileattach_library.Model.Item;
...
import com.orane.docassist.fileattach_library.adapter.NewQueryListAdapter;
...
public class New_queries_Fragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.query_new,
                container, false);
...
/*
                            new SweetAlertDialog(getActivity(), SweetAlertDialog.WARNING_TYPE)
                                    .setTitleText("Priority Query Found")
                                    .setContentText("Please answer the priority queries first, before answering this")
                                    .setConfirmText("OK")
                            ...
                            Toast.makeText(getActivity(), "Please answer the priority queries first, before answering this", Toast.LENGTH_SHORT).show();
        ...
        if (isInternetOn()) {
            try {
                if ((Model.id) != null && !(Model.id).isEmpty() && !(Model.id).equals("null") && !(Model.id).equals("")) {
                    //----------------------- Doctor Inbox -------------------------------
                    params = Model.BASE_URL + "sapp/qinboxDoc?os_type=android&user_id=" + (Model.id) + "&browser_country=" + Model.browser_country + "&page=1&token=" + Model.token + "&enc=1";
                    //params = Model.BASE_URL + "sapp/qinboxDoc?user_id=410443&browser_country=" + (Model.browser_country) + "&page=1&token=" + Model.token;
                    System.out.println("params-----------" + params);
                    new MyTask_server().execute(params);
                    //----------------------- Doctor Inbox -------------------------------
...
/*                            if ((jsonobj1.getString("do_not_answer")).equals(JSONObject.NULL)) {
                        ...
                        if (null == arrayOfList || arrayOfList.size() == 0) {
                            ...
                            try {
                                //Intent ij = new Intent(getActivity(), POD_NewQueriesActivity.class);
                                Intent ij = new Intent(getActivity(), POD_Queries_Activity_New.class);
                                startActivity(ij);
                                getActivity().finish();
                            } catch (Exception e) {
                                e.printStackTrace();
...
*/
                    ...
                    try {
                        ...
                        params.putString("Details", json_err_feedback.toString());
                        Model.mFirebaseAnalytics.logEvent("New_Query_Not_Respond_Issue", params);
                        //------------ Google firebase Analitics--------------------
    ...
    public void setAdapterToListview() {
        try {
            objAdapter = new NewQueryListAdapter(getActivity(), R.layout.query_new_row, arrayOfList);
    ...
    public void go_back_msg() {
       /* new SweetAlertDialog(getActivity(), SweetAlertDialog.WARNING_TYPE)
                .setTitleText("")
                .setContentText("Oops.! Something went wrong. Please go back and Try again.")
                .setConfirmText("Ok.!")
                .setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                    @Override
                    public void onClick(SweetAlertDialog sDialog) {
        ...
        Toast.makeText(getActivity(), "Oops.! Something went wrong. Please go back and Try again.", Toast.LENGTH_SHORT).show();
...
/*                //------------ Tracker ------------------------
                ...
                Model.mFirebaseAnalytics = FirebaseAnalytics.getInstance(getActivity());
                Bundle params = new Bundle();
                params.putString("Answering_Status", str_response);
